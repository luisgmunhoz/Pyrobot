...
def calibrate(pin,baseVoltage):
    sensor = MQ2(pinData = pin, baseVoltage)
    print("Calibrating")
    sensor.calibrate()
    ...
    return True
def smoke():
    return sensor.readSmoke()
def LPG():
    return sensor.readLPG()
def methane():
    return sensor.readMethane()
def hydrogen():
    return sensor.readSHydrogen()
